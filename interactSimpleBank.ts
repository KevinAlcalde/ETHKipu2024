...
const simpleBankAddress = "0x603574F444a8683f5f74fD2B2a6e324Daa40c8a1";
...
async function main() {
  try {
    ...
    const registerTx = await simpleBankContract.register("Kevin", "Alcalde");
    await registerTx.wait(); // Wait for the transaction to be mined
    console.log("User registered successfully!");

    // 2. Deposit ETH into the contract
    console.log("Depositing ETH...");
    const depositAmount = ethers.parseEther("0.04"); // Deposit 0.1 ETH
    ...
    const withdrawAmount = ethers.parseEther("0.01"); // Withdraw 0.05 ETH
